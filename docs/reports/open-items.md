...
| Fixture scaffold-from-empty, generic validator, docs generation, worker-scope-against-the-real-fixture, and CI wiring for the Phase 3 fixture proof | Resolved: `worker_pipeline.py`, `scripts/worker_task_types.json`, and all 5 Rashi renderer/shard tools are now module-aware (Phase 3 Steps 3A-3D). Remaining, per `docs/reports/phase3-inventory.md`'s Step 8 section (6 of 38 acceptance rows): (1) no test proves the documented onboarding process reproduces the fixture from an empty state; (2) no generic, module-selectable validator has been run against the fixture; (3) no fixture-specific docs-generation script exists; (4) the `<module>`-templating worker-scope mechanism is proven only via a synthetic in-memory fixture, not the literal committed `demotractate` module; (5) the end-to-end onboarding proof (`npm run test:fixture-onboarding`) is manual/on-demand, not wired into automated CI. None require touching Yoma content or selecting a real second tractate. |
