...
|---|---|---|
| `argumentFlow.sourceRefs` normalization | **Contract formalized; mechanical repairs (PR 3) and the judgment-required pass (PRs 1+2) both applied, at VERSION 15.363 (Phase 2 of `docs/platform-closure-plan.md`).** The canonical discriminated-union contract (object form with a controlled sourceType vocabulary including an explicit `unknown` state, or the legacy string form) is documented and enforced by `validate_source_refs.py`'s `OBJECT_SOURCETYPE_INVALID` check (`docs/reports/sourcerefs-contract-decision.md`). Of the original 550 defective refs: **412 mechanical repairs applied** (`apply_sourcerefs_mechanical_repair.py`, byte-for-byte surgical and lossless); of the remaining 138 judgment-required refs, **105 resolved** by reading each step's text against the actual Gemara content on its daf (`apply_sourcerefs_semantic_repair.py`, full evidence per case in `docs/reports/source-refs-semantic-review.json`), leaving **33 genuinely unresolved** (content not found anywhere on the declared daf, content living on a different daf than declared, or a real tie between two equally-supported candidates - each documented individually, none forced). The 331 sound string refs stay in string form since converting them would require inventing `sourceType`. | **33 residual refs**: no further mechanical or textual-evidence repair is available from repository data alone; unblocking would require either an operator decision to move misplaced content between daf files (out of scope for a sourceRefs-only pass) or accepting the 33 as a permanent, documented residue. PR 4 (string-ref conversion decision) is a separate open operator decision, tracked in `docs/reports/source-refs-normalization-plan.md`. |
