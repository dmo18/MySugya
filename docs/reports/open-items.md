...
|---|---|---|
| `argumentFlow.sourceRefs` normalization | **Contract formalized; mechanical repairs (PR 3) and the judgment-required pass (PRs 1+2) both applied. The 331-string-conversion question and the mishnah/mishna vocabulary question (both formerly "PR 4") are now decided, not open** (`docs/reports/sourcerefs-contract-decision.md`). The canonical discriminated-union contract (object form with a controlled sourceType vocabulary including an explicit `unknown` state, or the legacy string form) is documented and enforced by `validate_source_refs.py`'s `OBJECT_SOURCETYPE_INVALID` check. Of the original 550 defective refs: **412 mechanical repairs applied** (`apply_sourcerefs_mechanical_repair.py`, byte-for-byte surgical and lossless); of the remaining 138 judgment-required refs, **105 resolved** by reading each step's text against the actual Gemara content on its daf (`apply_sourcerefs_semantic_repair.py`, full evidence per case in `docs/reports/source-refs-semantic-review.json`), leaving **33 genuinely unresolved, across 16 daf, 23 sugyot, 33 argumentFlow steps** (content not found anywhere on the declared daf, content living on a different daf than declared, or a real tie between two equally-supported candidates - each documented individually, none forced). The 331 sound string refs stay in string form permanently: converting them would require inventing `sourceType`, and string form is a first-class canonical shape, not a legacy one awaiting cleanup. `mishnah`/`mishna` are not the same field (sourceType, source-line `kind`, and one legacy argumentFlow `type` value are three separate, already-consistent vocabularies) and are not unified. | **33 residual refs only**: no further mechanical or textual-evidence repair is available from repository data alone; unblocking requires case-by-case classification and either a proven qualified cross-daf reference, a proven local repair, a documented removal where a step is legitimately unanchored, or accepting a genuine tie/absence as permanent documented residue (Phase 2B, Steps 2-4 of the current campaign, tracked in `docs/reports/source-refs-normalization-plan.md`). |
