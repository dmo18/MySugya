...
|---|---|---|
| `argumentFlow.sourceRefs` normalization | **Contract formalized and PR 3 (mechanical repairs) applied at VERSION 15.360 (Phase 2 of `docs/platform-closure-plan.md`).** The canonical discriminated-union contract (object form with a controlled sourceType vocabulary including an explicit `unknown` state, or the legacy string form) is documented and enforced by `validate_source_refs.py`'s `OBJECT_SOURCETYPE_INVALID` check (`docs/reports/sourcerefs-contract-decision.md`). Of the original 550 defective refs, **412 mechanical repairs are applied**: `apply_sourcerefs_mechanical_repair.py` rewrote exactly the `lineId` field on exactly 412 refs across 52 daf, proven byte-for-byte surgical (412 insertions/412 deletions, every changed line a `lineId` line) and lossless (`vilnaLine`/`sourceType`/`note` unchanged on every ref). **138 remain**, needing a human reading the step text against the Gemara; the 331 sound string refs stay in string form since converting them would require inventing `sourceType`. | Sequential judgment-required structural-repair PRs (1 and 2), tracked in `docs/reports/source-refs-normalization-plan.md`. |
